    ...
    for _, Balloon in pairs(Library.Network.Invoke("BalloonGifts_GetActiveBalloons")) do
        ...
        if Balloon.Id then
            while Library.Network.Invoke("BalloonGifts_GetActiveBalloons")[Balloon.Id] do
                ...
                -- Move to the position of the active balloon
                HRP.CFrame = CFrame.new(Balloon.Position)

                -- Check if the character is close to the balloon's position
                local distanceToBalloon = (Balloon.Position - HRP.Position).Magnitude
                if distanceToBalloon < 3 then
                    -- Move slightly downwards to the balloon's landing position
